...
@push('styles')
    <style>
        ...
        .kpi-card.kpi-nb::before {
            background: #6366f1;
        }

        .kpi-card.kpi-rem::before {
            background: #f59e0b;
        }
        ...
        .resume-table .val-success {
            color: #059669;
        }

        .resume-table .val-warning {
            color: #d97706;
        }
        ...
        @media(max-width:768px) {
            ...
            .rv-table td:nth-child(n+3):nth-child(-n+5) {
                display: none;
            }
        }
    </style>
...
@section('content')
    <div class="rv-wrap">
        ...
        <div class="rv-page-hd">
            <div class="rv-page-hd-left">
                <h1><i class="fas fa-chart-line" style="opacity:.4;font-size:1.1rem;margin-right:8px"></i>Rapport des ventes
                </h1>
                <p>
    ...
    <div class="kpi-card kpi-ca">
        ...
        <div class="kpi-val">{{ number_format($totalCABas, 0, ',', ' ') }} <sup>FCFA</sup></div>
        <div class="kpi-label">CA Caisse Basse</div>
        <div class="kpi-foot">
            <i class="fas fa-calendar-alt" style="font-size:9px"></i>
            {{ \Carbon\Carbon::parse($dateDebut)->format('d/m') }} –
            {{ \Carbon\Carbon::parse($dateFin)->format('d/m/Y') }}
        </div>
    </div>
    <div class="kpi-card kpi-nb">
        <div class="kpi-ico"><i class="fas fa-cash-register"></i></div>
        <div class="kpi-val">{{ number_format($totalCAHaut, 0, ',', ' ') }} <sup>FCFA</sup></div>
        <div class="kpi-label">CA Caisse Haute</div>
        <div class="kpi-foot">
            <i class="fas fa-calendar-alt" style="font-size:9px"></i>
            {{ \Carbon\Carbon::parse($dateDebut)->format('d/m') }} –
            {{ \Carbon\Carbon::parse($dateFin)->format('d/m/Y') }}
                ...
                <table class="resume-table">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="val">
                                {{ \Carbon\Carbon::parse($dateDebut)->diffInDays(\Carbon\Carbon::parse($dateFin)) + 1 }}
                                jours
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td class="val">{{ $nbVentes > 0 ? number_format($totalCA / $nbVentes, 0, ',', ' ') : 0 }}
                                FCFA</td>
                        </tr>
                        <tr>
                        ...
                        <tr class="resume-highlight">
                            <td class="lbl" style="color:#059669;font-weight:700"><i class="fas fa-trophy"
                                    style="font-size:10px;margin-right:4px"></i> Meilleur jour</td>
                            <td class="val">
                                @if ($ventesParJour->count() > 0)
                                    <div class="best-day-badge">
                                        ...
                                        <span class="best-day-ca">{{ number_format($ventesParJour->max(), 0, ',', ' ') }}
                                            FCFA</span>
                                    </div>
                                @else
                ...
                <table class="rv-table">
                            ...
                            <tr class="row-anim" style="animation-delay:{{ $i * 0.025 }}s">
                                <td>
                                    <a href="{{ route('factures.show', $facture) }}" class="facture-num">
                                        <i class="fas fa-receipt"
                                            style="font-size:9px;opacity:.5;margin-right:4px"></i>{{ $facture->numero }}
                                    </a>
                                </td>
                                ...
                                <td>
                                    <span class="paiement-badge">
                                        @if (str_contains(strtolower($facture->mode_paiement_label ?? ''), 'espèce') ||
                                                str_contains(strtolower($facture->mode_paiement_label ?? ''), 'cash'))
                                            <i class="fas fa-money-bill-wave" style="font-size:8px"></i>
                                        @elseif(str_contains(strtolower($facture->mode_paiement_label ?? ''), 'mobile') ||
                                                str_contains(strtolower($facture->mode_paiement_label ?? ''), 'momo'))
                                ...
                                <td>
                                    @php
                                        ...
                                        $tpClass =
                                            str_contains($tp, 'gros') && !str_contains($tp, 'demi')
                                                ? 'tp-gros'
                                                : (str_contains($tp, 'moyen') || str_contains($tp, 'demi')
                                                    ? 'tp-moyen'
                                                    : 'tp-detail');
                                    @endphp
                                    <span
                                        class="type-prix-badge {{ $tpClass }}">{{ $facture->type_prix_label }}</span>
                                </td>
                                ...
                                <td style="text-align:right">
                                    @if ($facture->remise > 0)
                                        <span class="montant-neg">-{{ number_format($facture->remise, 0, ',', ' ') }}
                                            F</span>
                                    @else
                                ...
                                <td>
                                    <div class="caissier-chip">
                                        <div class="caissier-avatar">
                                            {{ strtoupper(substr($facture->user->prenom ?? '?', 0, 1)) }}</div>
                                        {{ $facture->user->prenom ?? '—' }}
                                    </div>
                                </td>
                                <td>
                                ...
                                <td style="text-align:center;white-space:nowrap">
                                    <div
                                        style="display:inline-flex;align-items:center;gap:4px;background:#f8fafc;border:1px solid #f1f5f9;border-radius:10px;padding:3px">
                                        <a href="{{ route('factures.show', $facture) }}" class="act-btn act-btn-blue"
                                            title="Voir la facture"><i class="fas fa-eye"></i></a>
                                        <a href="{{ route('factures.pdf', $facture) }}" class="act-btn act-btn-red"
                                            title="Télécharger PDF" target="_blank"><i class="fas fa-file-pdf"></i></a>
                                    </div>
                                </td>
                            </tr>
                        @empty
                    ...
                    @if ($nbVentes > 0)
                        <tfoot>
                            <tr>
                                <td colspan="4"
                                    style="text-align:right;color:#94a3b8;font-size:10px;text-transform:uppercase;letter-spacing:.5px">
                                    Totaux</td>
                                <td style="text-align:right">
                                    {{ number_format($factures->sum('sous_total'), 0, ',', ' ') }} F</td>
                                <td style="text-align:right;color:#d97706">-
                                    {{ number_format($totalRemises, 0, ',', ' ') }} F</td>
                                <td style="text-align:right;color:#059669">{{ number_format($totalCA, 0, ',', ' ') }} F
                                </td>
                                <td colspan="3"></td>
                            </tr>
                        </tfoot>
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            backgroundColor: values.map(function(v) {
                                return v === maxVal ? 'rgba(99,102,241,0.9)' :
