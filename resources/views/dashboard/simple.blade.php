...
{{-- ══════════════════════════════════════
     SECTION CAISSIER
══════════════════════════════════════ --}}
@if (auth()->user()->isCaissier() || auth()->user()->isCaissierHaut())
    <div class="dg-section mt-4 greeting-bar"
        style="background: linear-gradient(135deg, rgba(15,23,42,0.82) 0%, rgba(30,58,95,0.80) 60%, rgba(15,52,96,0.82) 100%), url('{{ asset('image/caisse1.jpg') }}') center/cover no-repeat;">
...
@if (auth()->user()->isSuperviseur())
    <div class="dg-section mt-2 greeting-bar"
        style="background: linear-gradient(135deg, rgba(15,23,42,0.82) 0%, rgba(30,58,95,0.80) 60%, rgba(15,52,96,0.82) 100%), url('{{ asset('image/caisse1.jpg') }}') center/cover no-repeat;">
        ...
        document.querySelectorAll('.dg-card').forEach(card => {
            card.addEventListener('mousemove', function(e) {
                ...
                this.style.transform =
                    `translateY(-6px) scale(1.03) rotateX(${-y * 8}deg) rotateY(${x * 8}deg)`;
            });
